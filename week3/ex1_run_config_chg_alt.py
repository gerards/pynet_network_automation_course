...
def send_notification(device_name):
    ...
    sender = 'gerard@sharpe.net.au'
    recipient = 'gerard@sharpe.net.au'
...
def main():
    ...
    rtr1_ip_addr = '184.105.247.70' #raw_input("Enter pynet-rtr1 IP: ")
    rtr2_ip_addr = '184.105.247.71' #raw_input("Enter pynet-rtr2 IP: ")
    my_key = 'galileo1' #getpass(prompt="Auth + Encryption Key: ")
    ...
    for a_device in (pynet_rtr1, pynet_rtr2):
        ...
        if DEBUG:
            print " *DEBUG* Connected to device = {0}".format(device_name)
            print " *DEBUG* Last changed timestamp = {0}".format(last_changed)
            print " *DEBUG* Uptime = {0}".format(uptime)
        ...
        if device_name in saved_devices:
            ...
            if DEBUG:
                print " *DEBUG* %-15s: %s\n *DEBUG* %-15s: %i" % \
                      ("Device name", device_name, "Length of Name", len(device_name))
